### script to make plots of AIJ equilibrium data ###
...
for(ifile in 1:length(datList)) {
    ...
    for(var in 1:length(varList)) {
        sub <- subset(data, Variable %in% varList[var])
        ...
        if(var == 2 | var == 3 | var == 4 | var == 8 | var == 9) {
            ### LOW LATITUDE MEANS ###
            ...
            ggsave(paste0(run, "_EqPlot_", varList[var], '_LoFacet_', format(Sys.time(), "%y%m%d"), ".png"), height = 6.25, width = 5, unit = 'in', dpi = 300)
            ...
            ggsave(paste0(run, "_EqPlot_", varList[var], '_MidFacet_', format(Sys.time(), "%y%m%d"), ".png"), height = 6.25, width = 5, unit = 'in', dpi = 300)
            ...
            ggsave(paste0(run, "_EqPlot_", varList[var], '_HiFacet_', format(Sys.time(), "%y%m%d"), ".png"), height = 7, width = 5, unit = 'in', dpi = 300)
