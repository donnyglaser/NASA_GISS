...
if(length(list.files(getwd(), pattern='STOPFLAG_aij.rds')) > 0) {
    goFlag <- FALSE
    print('STOP = TRUE')
} else if(length(list.files(getwd(), pattern='Temporary_aij_')) > 0) {
    ## Bring in settings from first eqAnalysis run ##
    iTab <- readRDS('Temporary_aij_IterationVariables.rds')
    outFileName <- readRDS('Temporary_aij_TableFileName.rds')
    outFrame <- readRDS(outFileName)
    diagI <- iTab[1]
    YearI <- iTab[2] + 1
    YearE <- max(dateList)
    file.remove(outFileName)
    file.remove('Temporary_aij_IterationVariables.rds')
    file.remove('Temporary_aij_TableFileName.rds')
...
if(goFlag == TRUE) {
    ...
    diagFiles <- list.files(getwd(), pattern = paste0('aij', runName))
    ...
    for(iYear in YearI:YearE) { # handles up to 9999 years
        ...
        for(iMon in 1:monEnd) {
            ...
            varList <- subset(allVar, Diagnostic %in% 'aij')
            for(iVar in 1:nrow(varList)) {
                t_data <- ncvar_get(nc_data, varid=varList[iVar,2])

                print(paste0('aij', ', ', iYear, ', ', iMon, ', ', iVar))
                ...
                for(iLat in 1:length(latList)) {
                    ...
                    tOut <- cbind(runName, iYear, sprintf('%02s', iMon), monList[iMon], latList[iLat], 'aij', varList[iVar,2], 'AtmosSurface', mean(t$value, na.rm=TRUE), sd(t$value, na.rm=TRUE))
                ...
                tOut <- cbind(runName, iYear, sprintf('%02s', iMon), monList[iMon], 'SouthernHemisphere', 'aij', varList[iVar,2], 'AtmosSurface', hemiData[1], 0)
                colnames(tOut) <- outNames
                outFrame <- rbind(outFrame, tOut)
                tOut <- cbind(runName, iYear, sprintf('%02s', iMon), monList[iMon], 'NorthernHemisphere', 'aij', varList[iVar,2], 'AtmosSurface', hemiData[2], 0)
                colnames(tOut) <- outNames
                outFrame <- rbind(outFrame, tOut)
                tOut <- cbind(runName, iYear, sprintf('%02s', iMon), monList[iMon], 'Global', 'aij', varList[iVar,2], 'AtmosSurface', hemiData[3], 0)
                colnames(tOut) <- outNames
                outFrame <- rbind(outFrame, tOut)
        ...
        if(as.numeric(difftime(Sys.time(),startTime,units='hours')) > 11.25) {
                ...
                saveRDS(c('aij', iYear), file = 'Temporary_aij_IterationVariables.rds')

                colnames(outFrame) <- outNames
                outFrame[,c(2:3,9:10)] <- apply(outFrame[,c(2:3,9:10)], 2, as.numeric)

                saveRDS(outFrame, file = paste0(runName, '_EqPARTIAL_aij_', format(Sys.time(), "%y%m%d"), '.rds'))
                saveRDS(paste0(runName, '_EqPARTIAL_aij_', format(Sys.time(), "%y%m%d"), '.rds'), file = 'Temporary_aij_TableFileName.rds')
...
if(goFlag == TRUE) {
    ...
    saveRDS(outFrame, file = paste0(runName, '_Equilibrium_aij_', format(Sys.time(), "%y%m%d"), '.rds'))
    saveRDS('STOP', 'STOPFLAG_aij.rds')
    goFlag <- FALSE
} else {
