...
if(length(list.files(getwd(), pattern='STOPFLAG_aijk.rds')) > 0) {
    goFlag <- FALSE
    print('STOP = TRUE')
} else if(length(list.files(getwd(), pattern='Temporary_aijk_')) > 0) {
    ## Bring in settings from first eqAnalysis run ##
    iTab <- readRDS('Temporary_aijk_IterationVariables.rds')
    outFileName <- readRDS('Temporary_aijk_TableFileName.rds')
    outFrame <- readRDS(outFileName)
    diagI <- iTab[1]
    YearI <- iTab[2] + 1
    YearE <- max(dateList)
    file.remove(outFileName)
    file.remove('Temporary_aijk_IterationVariables.rds')
    file.remove('Temporary_aijk_TableFileName.rds')
...
if(goFlag == TRUE) {
    ...
    diagFiles <- list.files(getwd(), pattern = paste0('aijk', runName))
    ...
    for(iYear in YearI:YearE) { # handles up to 9999 years
        ...
        for(iMon in 1:monEnd) {
            ...
            varList <- subset(allVar, Diagnostic %in% 'aijk')
            for(iVar in 1:nrow(varList)) {
                t_data <- ncvar_get(nc_data, varid=varList[iVar,2])

                print(paste0('aijk', ', ', iYear, ', ', iMon, ', ', iVar))
                ...
                for(iLayer in 1:length(aLayers)) {
                    ...
                    for(iLat in 1:length(latList)) {
                        ...
                        tOut <- cbind(runName, iYear, sprintf('%02s', iMon), monList[iMon], latList[iLat], 'aijk', varList[iVar,2], paste0('OceanLayer_', aLayers[iLayer]), mean(t$value, na.rm=TRUE), sd(t$value, na.rm=TRUE))
        ...
        if(as.numeric(difftime(Sys.time(),startTime,units='hours')) > 11.25) {
                ...
                saveRDS(c('aijk', iYear), file = 'Temporary_aijk_IterationVariables.rds')

                colnames(outFrame) <- outNames
                outFrame[,c(2:3,9:10)] <- apply(outFrame[,c(2:3,9:10)], 2, as.numeric)

                saveRDS(outFrame, file = paste0(runName, '_EqPARTIAL_aijk_', format(Sys.time(), "%y%m%d"), '.rds'))
                saveRDS(paste0(runName, '_EqPARTIAL_aijk_', format(Sys.time(), "%y%m%d"), '.rds'), file = 'Temporary_aijk_TableFileName.rds')
...
if(goFlag == TRUE) {
    ...
    saveRDS(outFrame, file = paste0(runName, '_Equilibrium_aijk_', format(Sys.time(), "%y%m%d"), '.rds'))
    saveRDS('STOP', 'STOPFLAG_aijk.rds')
    goFlag <- FALSE
